...
def add_HSV_filter(frame, camera):
    ...
    # # Left camera HSV range
    # l_b_l = np.array([143,110,50])
    # u_b_l = np.array([255,255,255])

    # Create binary mask
    if camera == 1:
        mask = cv2.inRange(hsv, (126,147,146), (180,255,255))
    else:
        mask = cv2.inRange(hsv, (126,147,146), (180,255,255))
...
cap_right = cv2.imread('/data/l (4).jpg')
cap_left = cv2.imread('/data/r (4).jpg')
