...
Distance (cm) received from the NodeMCU ESP8266 over USB serial is shown
on the frame.

STEREO INTEGRATION (from Code 1):
- Dual Picamera2(0) + Picamera2(1) stereo input
- StereoSGBM disparity map
- Hybrid K-Means + GrabCut segmentation mask
- HSV filtering as segmentation hint
- Kalman depth filter
- True stereo depth replaces fake D_static/tan() monocular
- Disparity + Hybrid Mask windows
"""
...
# ── Shared state (written by serial reader, read by VPython & CV loops) ───────
shared = {
...
def open_controller_serial():
    try:
        serial_link = serial.Serial(SERIAL_PORT, BAUD_RATE, timeout=0)
        time.sleep(2)
...
def send_angles(serial_link, pan_angle, tilt_angle):
    if serial_link is None:
        return
    try:
        serial_link.write(f"{pan_angle},{tilt_angle}\n".encode("utf-8"))
    except serial.SerialException as exc:
        print(f"Serial send failed: {exc}")


def read_imu_data(serial_link):
    if serial_link is None:
        return None

    latest = None
    try:
        while serial_link.in_waiting > 0:
            raw   = serial_link.readline().decode("utf-8", errors="replace").strip()
            ...
            latest = [f"{w:.4f}", f"{x:.4f}", f"{y:.4f}", f"{z:.4f}"]
    except Exception:
        pass
...
def find_red_center(frame):
    """Original HSV contour detector kept for angle/contour drawing."""
...
def opencv_thread():
    ...
    serial_link = open_controller_serial()
    ...
    while True:
        ...
        frame = frame_right.copy()   # display frame = right camera
        ...
        # ── IMU read + display ────────────────────────────────────────────────
        imu_data = read_imu_data(serial_link)
        ...
        if (pan_angle, tilt_angle) != last_sent_angles:
            send_angles(serial_link, pan_angle, tilt_angle)
...
# ═════════════════════════════════════════════════════════════════════════════
#  Main loop  (main thread — required by VPython)
# ═════════════════════════════════════════════════════════════════════════════
def main():
    ...
    try:
        while True:
            ...
            # ── Target sphere — uses ultrasonic distance ───────────────────────
            if dist is not None and dist > 0:
                VIS_SCALE    = 0.05
